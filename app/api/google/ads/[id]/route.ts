...
export const GET = asyncHandler(async (req: Request, { params }: { params: { id: string } }) => {
  ...
  const account = await prisma.googleAdsAccount.findUnique({ where: { id } });

  if (!account) {
    return ApiResponseHelper.notFound('Google Ads account not found');
  }

  return ApiResponseHelper.success(account);
});

const updateGoogleAdsAccountSchema = z.object({
...
export const PUT = asyncHandler(async (req: Request, { params }: { params: { id: string } }) => {
  ...
  const existing = await prisma.googleAdsAccount.findUnique({ where: { id } });
  if (!existing) {
    return ApiResponseHelper.notFound('Google Ads account not found');
  }

  const body = await req.json();
  const validatedData = updateGoogleAdsAccountSchema.parse(body);
